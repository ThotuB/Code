...
u8* Forward(u8 input[32]) {
    u8 *output = (u8*)calloc(32, sizeof(u8));
    ...
    for (u32 i = 0; i < 256; i++) {
        ...
        for (u8 j = 0; j < 32; j++) {
            int j = 0;
            for (u8 k = 0; k < 32; k++) {
                input[j] ^= output[k] * ((diffusion[j] >> k) & 0x1);
                printf("%02x - %d\n", input[0], (diffusion[j] >> k) & 0x1);
            }
        }
    }
    for (u8 i = 0; i < 16; i++) {
        output[i] = confusion[input[i * 2]] ^ 
                    confusion[input[i * 2 + 1] + 256];
    }

    return output;
}

u8* Backward(u8 target[32]) {
    u8 *input = (u8*)malloc(32 * sizeof(u8));
}
